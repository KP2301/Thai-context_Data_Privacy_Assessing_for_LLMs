...
def parse_file_content(file_path):
    ...
    try:
        ...
        for sample in samples[1:]:  # Skip first empty split
            ...
            token_match = re.search(r'PROMPT TOKENS:\s*(\d+)', sample)
            
            if status_match and token_match:
                is_leaked = 'LEAKED' in status_match.group(1)
                token_count = int(token_match.group(1))
                results.append((token_count, is_leaked))
...
def extract_model_name(filename):
    ...
    token_counted_casual_kimi.txt -> Kimi
    """
    patterns = {
        'kimi': 'Kimi',
        'llama': 'Llama',
    ...
    return filename.replace('token_counted_', '').replace('.txt', '').title()


def aggregate_results_by_token_range(all_results, range_size=500):
    """
    Aggregate results by token length ranges
    
    Args:
        all_results: dict of {model: [(token_count, is_leaked), ...]}
    ...
    for model, results in all_results.items():
        for token_count, is_leaked in results:
            # Calculate range bucket
            range_start = (token_count // range_size) * range_size
...
def plot_clustered_bar_chart(asr_results, range_size, output_path='asr_by_token_length.png'):
    """
    Create clustered bar chart showing ASR by token length range for each model
    """
    ...
    # Customize chart
    ax.set_xlabel('Token Length Range', fontsize=12, fontweight='bold')
    ax.set_ylabel('Attack Success Rate (ASR %)', fontsize=12, fontweight='bold')
    ax.set_title(f'Attack Success Rate by Token Length Range\n(Range Size: {range_size} tokens)', 
...
def main():
    ...
    print("=" * 70)
    range_size = input("\nEnter token range size (default: 500): ").strip()
    range_size = int(range_size) if range_size.isdigit() else 500
    print(f"\n🔧 Using token range size: {range_size}")
    ...
    for round_name in rounds:
        ...
        for file_path in folder_path.glob("token_counted_*.txt"):
    ...
    # Aggregate results by token range
    print(f"\n🔄 Aggregating results by {range_size}-token ranges...")
    aggregated = aggregate_results_by_token_range(all_model_results, range_size)
    ...
    # Plot graph
    print("\n📊 Generating graph...")
    output_path = f"asr_by_token_length_{level}_{range_size}.png"
    plot_clustered_bar_chart(asr_results, range_size, output_path)
    
    # Display detailed results
    print("\n" + "=" * 70)
    print("📋 Detailed ASR by Token Range")
    print("=" * 70)
